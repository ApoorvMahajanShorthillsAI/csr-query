"""
Figure Extraction: Approval letter Latest.pdf
...
def process_approval_figures(pdf_path: Path, output_base_dir: Path):
    print(f"\n--- Extracting Figures from {pdf_path.name} ---")
    out_dir = output_base_dir / "Approval letter Latest"
    out_dir.mkdir(parents=True, exist_ok=True)

    with open(out_dir / "figure_analysis.json", "w", encoding="utf-8") as f:
        json.dump([], f, indent=2)
    print("  No figures detected in Approval letter Latest.pdf. Saved empty figure_analysis.json.")
